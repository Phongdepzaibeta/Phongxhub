...
object.phongHub = instance.new("ScreenGui", {
    Name = "Mo Mo Be";
    Parent = game.CoreGui;
...
pcall(function()
    if true and isfile('phong.bin') and not getgenv().Key then
        object.TextBox.Text = readfile("phong.bin")
...
local function SaveKey()
    local s, e = pcall(function()
        if not isfile('phong.bin') then
            writefile('phong.bin', GetKeyText())
        end
    end)
end
...
local function wl(key)
    ...
    if sign_match == decode_response.sign then
        ...
        if IsMobile then 
            ...
            ImageLabel.Image = "rbxassetid://75474257824650"
        ...
        local Window =
            Fluent:CreateWindow(
            {
                Title = "phong Hub Update 20",
                SubTitle = "tạo bởi phong đẹp trai nhất thế giới",
                TabWidth = 160,
        ...
        function Window:NewTab(mmb)
            ...
            function newmm:NewParagraph(cf)
                local paragraphnew
                newTitle = cf.Title or "phong Hub"
                ...
                function paragraphnew:Set(cf2)
                    newTitle = cf2.Title or "phong Hub"
        ...
        function SaveSettings2()
            local HttpService = game:GetService("HttpService")
            if not isfolder("phong Hub") then
                makefolder("phong Hub")
            end
            writefile("phong Hub/" .. SaveFileName2, HttpService:JSONEncode(Settings2))
        end
        ...
        function ReadSetting2()
            local s, e =
                ...
                function()
                    ...
                    return HttpService:JSONDecode(readfile("phong Hub/" .. SaveFileName2))
                end
            )
            if s then
                return e
        ...
        Fluent:Notify(
            {
                Title = "phong Hub",
                Content = "phong là người đẹp trái nhất.",
                Duration = 15
            }
        )
            else
